...
<template>
  <Dialog :open="open" @update:open="(value) => !value && !loading && emit('close')">
    <DialogFixedContent title="推送渠道" :description="`${projectName} 的评审结果将发送到已启用渠道。`" class="sm:max-w-3xl">
      <div v-if="detailsLoading" class="flex min-h-64 items-center justify-center gap-2 text-sm text-muted-foreground">
      ...
      <div v-else class="grid gap-5 md:grid-cols-[190px_1fr]">
        ...
        <div v-if="current" class="space-y-4 rounded-md border p-4">
          ...
          <div v-if="current.type !== 'wechat_bot'" class="space-y-1.5">
            ...
            <div class="relative">
              <Input v-model="current.config.webhookUrl" :type="webhookVisible ? 'text' : 'password'" class="pr-10" autocomplete="new-password" placeholder="粘贴机器人 Webhook 地址" />
              <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-foreground" :aria-label="webhookVisible ? '隐藏 Webhook 地址' : '显示 Webhook 地址'" @click="webhookVisible = !webhookVisible">
          ...
          <div v-if="current.type === 'dingtalk_robot'" class="space-y-1.5">
            <Label>加签密钥（可选）</Label>
            <div class="relative">
              <Input v-model="current.config.secret" :type="secretVisible ? 'text' : 'password'" class="pr-10" autocomplete="new-password" placeholder="未开启加签可留空" />
              <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-foreground" :aria-label="secretVisible ? '隐藏加签密钥' : '显示加签密钥'" @click="secretVisible = !secretVisible">
      ...
      <template #footer><Button variant="outline" :disabled="loading" @click="emit('close')">取消</Button><Button :disabled="loading || detailsLoading" class="gap-1.5" @click="save"><Check class="h-4 w-4" />保存渠道</Button></template>
    </DialogFixedContent>
  </Dialog>
</template>
