...
2. 当前会话窗口按联系人最后一条入站消息计算，超过 24 小时后管理端拒绝发送。
3. 扫码确认可能返回新的消息服务 `base_url`，必须和 Bot Token 一起持久化。
4. `get_updates_buf` 每次变化都持久化，服务重启时作为 SDK `InitialBuf` 恢复。
5. SDK 可能重复投递消息；模块以 Bot、方向和外部消息 ID 做幂等约束。
6. 二维码状态可能要求切换轮询主机，扫码会话会保存并使用重定向地址。
