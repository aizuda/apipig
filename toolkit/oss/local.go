...
// UploadFile 上传文件
func (l *Local) UploadFile(file *multipart.FileHeader) (string, string, string, error) {
	// 读取文件后缀
	ext := path.Ext(file.Filename)
	// 读取文件名并加密
	name := strings.TrimSuffix(file.Filename, ext)
	name = toolkit.GetByteMd5([]byte(name))
	yearMonth := time.Now().Format("200601")
	// 尝试创建此路径
	mkdirErr := os.MkdirAll(filepath.Join(l.uploadPath, yearMonth), os.ModePerm)
	if mkdirErr != nil {
	...
	// 路径/年月/文件名
	p := filepath.Join(l.uploadPath, yearMonth, filename)
	...
	return p, toolkit.GetFileMd5(p), ext, nil
}
...
func (l *Local) DeleteFile(key string) error {
	p := filepath.Join(l.uploadPath, key)
	if strings.Contains(p, l.uploadPath) {
